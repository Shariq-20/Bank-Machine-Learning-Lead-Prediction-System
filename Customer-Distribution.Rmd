...
ggplot(Data_DEX)+ geom_boxplot(mapping= aes(x=Account_Type, y= Total_Accounts , colour = as.factor(Account_Type)), width= 0.5) + facet_grid(~Occupation)+ labs(title = "Account Type by Ocupation") +
  ylab("Account Holders") + xlab("Account Type")
...
bestRF <-  randomForest(Target~., undersampled_n, mtry = 7, nodesize = 1)
...
ggplot(data_1, aes(x = Age_group, fill = Vintage_group)) + 
  geom_bar() +
